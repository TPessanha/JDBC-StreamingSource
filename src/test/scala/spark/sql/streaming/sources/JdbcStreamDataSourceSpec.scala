...
import utils.{TemporaryDirEnv, UnitSpec}

import java.sql.{Connection, Date, DriverManager, Timestamp}
import scala.collection.JavaConverters._

class JdbcStreamDataSourceSpec extends UnitSpec with TemporaryDirEnv with DataFrameComparer {
  ...
  private val dataSourceFormat = "jdbc-streaming"
  ...
  private def getDirs(uid: String = java.util.UUID.randomUUID.toString): (String, String) = {
    val baseDir = getRandomDir(uid)
    (s"$baseDir/checkpoint", s"$baseDir/output")
